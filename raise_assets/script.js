(function($) {
  ...
  $("#contact-submit").click(function(e) {
    ...
    if (error == false) {
      ...
      // make the request
      fetch("https://stark-brook-91785.herokuapp.com/sendemail", {
        method: "POST",
        body: formData
      }).then(result => {
  ...
  $("#subscribe-btn").click(function(e) {
    ...
    if (email.length == 0 || email.indexOf("@") == "-1") {
      var error = true;
      $("#subscribe-email").css("border-color", "#D8000C");
    } else {
      $("#subscribe-email").css("border-color", "#666");
    }
    ...
    if (error == false) {
      ...
      var formData = new FormData();
      formData.append("name", "Subscribe");
      formData.append("email", email);
      formData.append("subject", "Subscribe");
      formData.append("message", "A client subscribed: " + email);
      // make the request
      fetch("https://stark-brook-91785.herokuapp.com/sendemail", {
        method: "POST",
        body: formData
        ...
        result.text().then(result => {
          if (result == "sent") {
            $("#subscribe-btn").html("THANK YOU");
            $("#subscribe-btn").attr({ disabled: "true" });
            $("#subscribe-email").val("");
            $("#subscribe-email").attr({ disabled: "true" });
          } else {
            $("#subscribe-btn").attr("value", "SUBSCRIBE NOW");
          }
        });
      });
    }
